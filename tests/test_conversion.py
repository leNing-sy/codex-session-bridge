...
class WindsurfTests(unittest.TestCase):
    ...
    def _make_encrypted_pb(plaintext: bytes) -> bytes:
        """Encrypt plaintext with the Windsurf AES-256-GCM key."""
    ...
    def _encode_tag(field_no: int, wire_type: int) -> bytes:
        return WindsurfTests._encode_varint((field_no << 3) | wire_type)

    @staticmethod
    def _encode_string(field_no: int, text: str) -> bytes:
        encoded = text.encode("utf-8")
        return (WindsurfTests._encode_tag(field_no, 2) +
                WindsurfTests._encode_varint(len(encoded)) +
                encoded)

    @staticmethod
    def _encode_varint_field(field_no: int, value: int) -> bytes:
        return WindsurfTests._encode_tag(field_no, 0) + WindsurfTests._encode_varint(value)

    @staticmethod
    def _encode_message(field_no: int, data: bytes) -> bytes:
        return (WindsurfTests._encode_tag(field_no, 2) +
                WindsurfTests._encode_varint(len(data)) +
                data)

    @staticmethod
    def _make_metadata(ts_seconds: int) -> bytes:
        inner = WindsurfTests._encode_varint_field(1, ts_seconds)
        return WindsurfTests._encode_message(5, WindsurfTests._encode_message(1, inner))

    def test_windsurf_session_extraction(self) -> None:
        with tempfile.TemporaryDirectory() as root_name:
            root = Path(root_name)
            cascade_dir = root / ".codeium" / "windsurf" / "cascade"
            cascade_dir.mkdir(parents=True)

            session_id = "test-traj-001"
            # Build a minimal CortexTrajectory with 3 steps:
            # 1. user_input (variant 19): "Fix the bug"
            # 2. planner_response (variant 20): "Looking at it now"
            # 3. user_input (variant 19): "PS C:\\test> cd /d C:\\project"
            user_step = (
                WindsurfTests._encode_varint_field(1, 14) +
                WindsurfTests._encode_varint_field(4, 3) +
                WindsurfTests._make_metadata(1776873454) +
                WindsurfTests._encode_message(19, WindsurfTests._encode_string(2, "Fix the bug"))
            )
            assistant_step = (
                WindsurfTests._encode_varint_field(1, 14) +
                WindsurfTests._encode_varint_field(4, 3) +
                WindsurfTests._make_metadata(1776873455) +
                WindsurfTests._encode_message(20, WindsurfTests._encode_string(1, "Looking at it now"))
            )
            user_step2 = (
                WindsurfTests._encode_varint_field(1, 14) +
                WindsurfTests._encode_varint_field(4, 3) +
                WindsurfTests._make_metadata(1776873456) +
                WindsurfTests._encode_message(19, WindsurfTests._encode_string(2, "PS C:\\test> cd /d C:\\project"))
            )

            trajectory = (
                WindsurfTests._encode_string(1, session_id) +
                WindsurfTests._encode_message(2, user_step) +
                WindsurfTests._encode_message(2, assistant_step) +
                WindsurfTests._encode_message(2, user_step2) +
                WindsurfTests._encode_varint_field(4, 4)
            )

            encrypted = WindsurfTests._make_encrypted_pb(trajectory)
            (cascade_dir / f"{session_id}.pb").write_bytes(encrypted)

            from session_sdk.stores import WindsurfStore
            store = WindsurfStore(root / ".codeium" / "windsurf")
            session = store.load(session_id)
            self.assertEqual(session.provider, "windsurf")
            self.assertEqual(session.session_id, session_id)

            messages = MessageExtractor().from_windsurf(session)
            self.assertEqual(len(messages), 3)
            self.assertEqual(messages[0].role, "user")
            self.assertEqual(messages[0].text, "Fix the bug")
            self.assertEqual(messages[1].role, "assistant")
            self.assertEqual(messages[1].text, "Looking at it now")
            self.assertEqual(messages[1].provider, "windsurf")
            self.assertEqual(messages[2].role, "user")
            self.assertFalse(messages[2].is_contextual)

    def test_windsurf_to_pi_conversion(self) -> None:
        with tempfile.TemporaryDirectory() as root_name:
            root = Path(root_name)
            cascade_dir = root / ".codeium" / "windsurf" / "cascade"
            cascade_dir.mkdir(parents=True)
            ...
            session_id = "test-traj-002"
            user_step = (
                WindsurfTests._encode_varint_field(1, 14) +
                WindsurfTests._encode_varint_field(4, 3) +
                WindsurfTests._make_metadata(1776873454) +
                WindsurfTests._encode_message(19, WindsurfTests._encode_string(2, "Hello assistant"))
            )
            assistant_step = (
                WindsurfTests._encode_varint_field(1, 14) +
                WindsurfTests._encode_varint_field(4, 3) +
                WindsurfTests._make_metadata(1776873455) +
                WindsurfTests._encode_message(20, WindsurfTests._encode_string(1, "Hello user"))
            )
            trajectory = (
                WindsurfTests._encode_string(1, session_id) +
                WindsurfTests._encode_message(2, user_step) +
            ...
            from session_sdk.stores import WindsurfStore
            from session_sdk.converters import WindsurfToPiConverter
            windsurf_store = WindsurfStore(root / ".codeium" / "windsurf")
            pi_store = PiStore(pi_agent_home)
            dcp_store = PiDcpStore(pi_dcp_home)
            factory = SessionIdFactory(preserve_ids=True)
            converter = WindsurfToPiConverter(windsurf_store, pi_store, dcp_store, factory)
            plan = converter.plan(session_id)
            self.assertEqual(plan.source.provider, "windsurf")
            self.assertEqual(len(plan.records), 3)  # header + 2 messages
            self.assertEqual(plan.records[0]["type"], "session")
            self.assertEqual(plan.records[1]["type"], "message")
            self.assertEqual(plan.records[2]["type"], "message")
            converter.write(plan, overwrite=True)
            self.assertTrue(plan.destination.exists())
            self.assertTrue(plan.services[0].exists())

    def test_windsurf_list_sessions(self) -> None:
        with tempfile.TemporaryDirectory() as root_name:
            root = Path(root_name)
            cascade_dir = root / ".codeium" / "windsurf" / "cascade"
            cascade_dir.mkdir(parents=True)
            ...
            for sid in ("alpha", "beta"):
                trajectory = (
                    WindsurfTests._encode_string(1, sid) +
                    WindsurfTests._encode_varint_field(4, 4)
                )
                encrypted = WindsurfTests._make_encrypted_pb(trajectory)
                (cascade_dir / f"{sid}.pb").write_bytes(encrypted)
