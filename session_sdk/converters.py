...
class MessageExtractor:
    ...
    def from_codex(self, session: NativeSession) -> list[TextMessage]:
        ...
        for record in session.records:
            ...
            if msg:
                messages.append(msg)
        return self._dedupe_codex_user_messages(messages)

    @staticmethod
    def _dedupe_codex_user_messages(messages: list[TextMessage]) -> list[TextMessage]:
        # Codex re-appends a pending (unanswered) user input to the rollout every
        # time the session is resumed; keep only the first of each unanswered run.
        deduped: list[TextMessage] = []
        last_user_text: str | None = None
        for message in messages:
            if message.role == "user" and not message.is_contextual and not message.is_compaction:
                # The re-appended copy may differ in surrounding whitespace.
                if message.text.strip() == last_user_text:
                    continue
    ...
    # Claude Code interleaves the transcript with slash-command records, local
    # command output, system reminders, and interruption markers; none of these
    # are conversation content.
    _CLAUDE_NOISE_MARKERS: tuple[str, ...] = (
        "<command-name>",
        "<command-message>",
    ...
    @classmethod
    def _is_claude_noise(cls, text: str) -> bool:
        head = text.lstrip()[:120]
        return any(head.startswith(marker) for marker in cls._CLAUDE_NOISE_MARKERS)
    ...
    def from_claude(self, session: NativeSession) -> list[TextMessage]:
        ...
        for record in session.records:
            ...
            if rtype not in ("user", "assistant"):
                continue
            # Sidechain records belong to subagent conversations, and isMeta
            # records are harness bookkeeping; neither is part of the main thread.
            ...
            if record.get("isCompactSummary") is True:
                ...
                continue
            contextual = self._is_contextual(text) or self._is_claude_noise(text)
            messages.append(TextMessage(role, text, timestamp, model=model, is_contextual=contextual))
        return messages
